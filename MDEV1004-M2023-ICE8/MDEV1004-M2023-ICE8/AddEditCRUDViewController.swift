...
class AddEditCRUDViewController: UIViewController
{
    // UI References
    ...
    var movie: Movie?
    var crudViewController: CRUDViewController? // Updated from MovieViewController
    ...
    @IBAction func UpdateButton_Pressed(_ sender: UIButton)
    {
        ...
        if let movie = movie {
            requestType = "PUT"
            urlString = "https://livesite-lejp.onrender.com/api/update/\(movie._id)"
        } else {
            requestType = "POST"
            urlString = "https://livesite-lejp.onrender.com/api/add"
        }
